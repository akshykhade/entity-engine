  ...
  return (
    <div className="bg-background px-10 py-10 min-h-svh">
      <div className="space-y-6 mx-auto max-w-7xl">
        ...
        <div className="gap-px grid grid-cols-1 lg:grid-cols-[1fr_300px] bg-border/60 border border-border/60 rounded-2xl overflow-hidden">
          <FeaturedPane />
          <PortfolioPane />
        </div>

        <div className="gap-px grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] bg-border/60 border border-border/60 rounded-2xl overflow-hidden">
          <WatchlistTable />
          <NewsPane />
        </div>
      </div>
      ...
      <div className="flex justify-between items-baseline">
        <div>
          ...
          <div className="flex items-baseline gap-3 mt-2">
            <span className="font-mono tabular-nums text-5xl">
  ...
  return (
    <div className="bg-background p-6">
      ...
      <div className="flex items-baseline gap-2 mt-2">
        <span className="font-heading tabular-nums text-3xl">
          ${total.toLocaleString()}
        </span>
      </div>
      <div className="inline-flex items-baseline gap-1 mt-1 font-mono tabular-nums text-emerald-600 dark:text-emerald-400 text-sm">
      ...
      <div className="mt-5">
        ...
        <div className="flex mt-3 rounded-full h-3 overflow-hidden">
          {ALLOCATION.map((a) => (
            <div
              key={a.name}
        ...
        <ul className="space-y-1.5 mt-3 font-mono text-[11px]">
          {ALLOCATION.map((a) => (
...
function WatchlistTable() {
  return (
    <div className="bg-background">
      <div className="flex justify-between items-baseline px-5 py-3 border-border/40 border-b">
      ...
      <table className="w-full">
        <thead>
          <tr className="border-border/40 border-b font-mono text-[10px] text-muted-foreground text-left uppercase tracking-[0.25em]">
            <th className="px-5 py-2 font-normal">Symbol</th>
            <th className="px-5 py-2 font-normal text-right">Price</th>
            <th className="px-5 py-2 font-normal text-right">Change</th>
            <th className="px-5 py-2 font-normal">24h</th>
            <th className="px-5 py-2 font-normal text-right">Vol</th>
          </tr>
        </thead>
        <tbody>
          {WATCHLIST.map((t) => {
            ...
            return (
              <tr
                key={t.symbol}
                className="hover:bg-foreground/[0.02] border-border/30 border-b"
              >
                ...
                <td className="px-5 py-2.5 font-mono tabular-nums text-sm text-right">
                  ${t.price.toFixed(2)}
                </td>
                <td className="px-5 py-2.5 font-mono tabular-nums text-sm text-right">
                  <span
                ...
                <td className="px-5 py-2.5 font-mono tabular-nums text-[11px] text-muted-foreground text-right">
                  {t.vol}
                </td>
              </tr>
            );
